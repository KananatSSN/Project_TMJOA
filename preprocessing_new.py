...
def construct_name(folder_path):
    ...
    z_folders = [f for f in os.listdir(folder_path) if f.endswith(('_z', '_Z')) and os.path.isdir(os.path.join(folder_path, f))]

    if z_folders:
        dcm_path = os.path.join(folder_path, z_folders[0], "SLZ+000.dcm")
...
def segmentation(filepath, outputfolder):
    ...
    widget.deviceComboBox.setCurrentText("cuda")
...
def skeletonized_cropping(input_file, crop_size=50, output_folder=None, threshold=None):
    ...
    # reference_point = (data.shape[0]//2, data.shape[1], data.shape[2]//2) # Old ref point
    reference_point = (data.shape[0]//2, 0, 0)
...
def find_dcm_folder(input_folder):
    """
    Alternative implementation using os.walk
    """
    result_folders = []
    
    for root, dirs, files in os.walk(input_folder):
        # Check if any directory in current level ends with '_z'
        has_z_folder = any(dirname.endswith('_z') or dirname.endswith('_Z') for dirname in dirs)
...
def batch_preprocessing(input_folder, result_folder, start_from=0, crop_size=112, threshold=-3999):
    ...
    print(f"Found {files_count} dcm folders in {input_folder}")
...
main = batch_preprocessing(input_folder, result_folder, start_from = resume_from, crop_size=112, threshold=-1999)
